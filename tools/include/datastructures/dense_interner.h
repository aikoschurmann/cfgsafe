...
typedef struct {
    ...
    void* (*copy_func)(Allocator alloc, const void *data, size_t len);
 ...
 */
typedef void* (*CopyFunc)(Allocator alloc, const void *data, size_t len);
 ...
 */
void* string_copy_func(Allocator alloc, const void *data, size_t len);  // null-terminate

/**
 * @brief Copy bytes exactly (no null terminator).
 */
void* binary_copy_func(Allocator alloc, const void *data, size_t len);  // exact copy
 ...
 * @return pointer to newly created DenseInterner or NULL on error
 */
 ...
 */
InternResult *intern(DenseInterner *interner, Slice *slice, void* meta);
 ...
 */
void *intern_ptr(DenseInterner *interner, Slice *slice, void* meta);
 ...
 */
int  intern_idx(DenseInterner *interner, Slice *slice, void* meta);
 ...
 */
const char *interner_get_cstr(DenseInterner *interner, int idx);
 ...
 */
InternResult *interner_get_result(DenseInterner *interner, int idx);
...
void interner_foreach(const DenseInterner *interner,
                      InternerIterFn cb,
                      void *user);
